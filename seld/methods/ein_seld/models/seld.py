...
class EINV2_1(nn.Module):
    def __init__(self, cfg, dataset):
        ...
        if self.pe_enable:
            self.pe = PositionalEncoding(pos_len=100, d_model=512, pe_type='t', dropout=0.0)
        self.sed_mhsa = nn.ModuleList([
            nn.TransformerEncoder(nn.TransformerEncoderLayer(
                d_model=512, nhead=8, dim_feedforward=1024, dropout=0.2), num_layers=2),
            nn.TransformerEncoder(nn.TransformerEncoderLayer(
                d_model=512, nhead=8, dim_feedforward=1024, dropout=0.2), num_layers=2)
        ])
        self.doa_mhsa = nn.ModuleList([
            nn.TransformerEncoder(nn.TransformerEncoderLayer(
                d_model=512, nhead=8, dim_feedforward=1024, dropout=0.2), num_layers=2),
            nn.TransformerEncoder(nn.TransformerEncoderLayer(
                d_model=512, nhead=8, dim_feedforward=1024, dropout=0.2), num_layers=2)
        ])
        self.sed_fc = nn.ModuleList([
            nn.Linear(512, 14, bias=True),
            nn.Linear(512, 14, bias=True)
        ])
        self.doa_fc = nn.ModuleList([
            nn.Linear(512, 3, bias=True),
            nn.Linear(512, 3, bias=True)
        ])
        self.sed_act = nn.Sequential() # nn.Sigmoid()
        self.doa_act = nn.Tanh()

        self.init_weight()
    
    def init_weight(self):
        for layer in self.sed_fc:
            init_layer(layer)
        for layer in self.doa_fc:
            init_layer(layer)
    ...
    def forward(self, x):
        ...
        # cnn
        x_sed = self.sed_cnn[0](x_sed)
        x_doa = self.doa_cnn[0](x_doa)
        x_sed = torch.einsum('c, nctf -> nctf', self.softc[0][:, 0, 0], x_sed) + \
            torch.einsum('c, nctf -> nctf', self.softc[0][:, 0, 1], x_doa)
        x_doa = torch.einsum('c, nctf -> nctf', self.softc[0][:, 1, 0], x_sed) + \
            torch.einsum('c, nctf -> nctf', self.softc[0][:, 1, 1], x_doa)
        x_sed = self.sed_cnn[1](x_sed)
        x_doa = self.doa_cnn[1](x_doa)
        x_sed = torch.einsum('c, nctf -> nctf', self.softc[1][:, 0, 0], x_sed) + \
            torch.einsum('c, nctf -> nctf', self.softc[1][:, 0, 1], x_doa)
        x_doa = torch.einsum('c, nctf -> nctf', self.softc[1][:, 1, 0], x_sed) + \
            torch.einsum('c, nctf -> nctf', self.softc[1][:, 1, 1], x_doa)
        x_sed = self.sed_cnn[2](x_sed)
        x_doa = self.doa_cnn[2](x_doa)
        x_sed = torch.einsum('c, nctf -> nctf', self.softc[2][:, 0, 0], x_sed) + \
            torch.einsum('c, nctf -> nctf', self.softc[2][:, 0, 1], x_doa)
        x_doa = torch.einsum('c, nctf -> nctf', self.softc[2][:, 1, 0], x_sed) + \
            torch.einsum('c, nctf -> nctf', self.softc[2][:, 1, 1], x_doa)
        x_sed = self.sed_cnn[3](x_sed)
        x_doa = self.doa_cnn[3](x_doa)
        # x_sed = torch.einsum('c, nctf -> nctf', self.softc[3][:, 0, 0], x_sed) + \
        #     torch.einsum('c, nctf -> nctf', self.softc[3][:, 0, 1], x_doa)
        # x_doa = torch.einsum('c, nctf -> nctf', self.softc[3][:, 1, 0], x_sed) + \
        #     torch.einsum('c, nctf -> nctf', self.softc[3][:, 1, 1], x_doa)
        x_sed = x_sed.mean(dim=3) # (N, C, T)
        x_doa = x_doa.mean(dim=3) # (N, C, T)
        ...
        x_sed_1 = self.sed_mhsa[0](x_sed).transpose(0, 1) # (N, T, C)
        x_sed_2 = self.sed_mhsa[1](x_sed).transpose(0, 1) # (N, T, C)   
        x_doa_1 = self.doa_mhsa[0](x_doa).transpose(0, 1) # (N, T, C)
        x_doa_2 = self.doa_mhsa[1](x_doa).transpose(0, 1) # (N, T, C)
        # x_sed_1 = torch.einsum('c, ntc -> ntc', self.softc[4][:, 0, 0], x_sed_1) + \
        #     torch.einsum('c, ntc -> ntc', self.softc[4][:, 0, 1], x_doa_1)
        # x_doa_1 = torch.einsum('c, ntc -> ntc', self.softc[4][:, 1, 0], x_sed_1) + \
        #     torch.einsum('c, ntc -> ntc', self.softc[4][:, 1, 1], x_doa_1)
        # x_sed_2 = torch.einsum('c, ntc -> ntc', self.softc[5][:, 0, 0], x_sed_2) + \
        #     torch.einsum('c, ntc -> ntc', self.softc[5][:, 0, 1], x_doa_2)
        # x_doa_2 = torch.einsum('c, ntc -> ntc', self.softc[5][:, 1, 0], x_sed_2) + \
        #     torch.einsum('c, ntc -> ntc', self.softc[5][:, 1, 1], x_doa_2)

        # fc
        x_sed_1 = self.sed_act(self.sed_fc[0](x_sed_1))
        x_sed_2 = self.sed_act(self.sed_fc[1](x_sed_2))
        x_sed = torch.stack((x_sed_1, x_sed_2), dim=2)
        x_doa_1 = self.doa_act(self.doa_fc[0](x_doa_1))
        x_doa_2 = self.doa_act(self.doa_fc[1](x_doa_2))
        x_doa = torch.stack((x_doa_1, x_doa_2), dim=2)
        output = {
            'sed': x_sed,
            'doa': x_doa,
        }
